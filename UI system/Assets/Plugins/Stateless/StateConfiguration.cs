...
namespace Stateless
{
    public partial class StateMachine<TState, TTrigger>
    {
        ...
        public partial class StateConfiguration
        {
            ...
            /// <summary>
            /// Ignores triggers that are not permitted during state configuration. Using this method avoid throwing an exception when attempt to fire unpermitted trigger
            /// </summary>
            public void IgnoreNotPermittedTriggers()
            {
                var triggersToIgnore = Enum.GetValues(typeof(TTrigger)).OfType<TTrigger>().ToList();
                IEnumerable<TTrigger> permittedTriggers = _representation.PermittedTriggers;
                foreach (TTrigger trigger in permittedTriggers)
                {
                    if (triggersToIgnore.Contains(trigger))
                        triggersToIgnore.Remove(trigger);
                }

                for (var i = 0; i < triggersToIgnore.Count; i++)
                {
                    TTrigger trigger = triggersToIgnore[i];
